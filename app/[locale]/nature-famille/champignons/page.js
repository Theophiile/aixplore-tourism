  ...
  return (
    <div>
        ...
        <div className="container-custom max-w-4xl">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
            ...
            <div className="space-y-4 text-lg text-stone font-montserrat leading-relaxed">
              ...
              <p className="font-semibold text-stone-dark">{locale === 'fr' ? "Entre jeux d'observation, senteurs de sous-bois et cueillette douce, cette sortie sensorielle mêle découverte, partage et plaisir simple — un vrai moment cocooning en pleine nature." : "Between observation games, forest scents and gentle picking, this sensory outing combines discovery, sharing and simple pleasure — a true cocooning moment in nature."}</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container-custom max-w-4xl">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
            <h2 className="text-3xl font-playfair font-bold text-stone-dark mb-6">{locale === 'fr' ? "🍁 Thématique : Découvrir, observer, savourer l'automne" : "🍁 Theme: Discover, Observe, Savor Autumn"}</h2>
            <ul className="space-y-3 text-lg text-stone font-montserrat">
              <li className="flex items-start gap-3"><span className="text-2xl flex-shrink-0">🍄</span><span>{locale === 'fr' ? "Observer et reconnaître quelques champignons typiques de la forêt savoyarde." : "Observe and recognize some typical mushrooms of the Savoyard forest."}</span></li>
              <li className="flex items-start gap-3"><span className="text-2xl flex-shrink-0">🌰</span><span>{locale === 'fr' ? "Ramasser les noix tombées sur le sentier et comprendre leur cycle naturel." : "Collect walnuts fallen on the trail and understand their natural cycle."}</span></li>
              <li className="flex items-start gap-3"><span className="text-2xl flex-shrink-0">👃</span><span>{locale === 'fr' ? "Sentir, toucher, comparer les textures du sol et des feuilles." : "Smell, touch, compare the textures of soil and leaves."}</span></li>
              <li className="flex items-start gap-3"><span className="text-2xl flex-shrink-0">🍰</span><span>{locale === 'fr' ? "Partager un goûter convivial à base de noix en fin de balade." : "Share a friendly snack made with walnuts at the end of the walk."}</span></li>
            </ul>
          </motion.div>
        </div>
      </section>
      ...
      <section className="section-padding bg-gradient-to-br from-lake-light to-gold-light">
        <div className="container-custom max-w-4xl">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
            ...
            <h3 className="text-2xl font-playfair font-bold text-lake-dark mb-4">{locale === 'fr' ? "🔎 Balade découverte & atelier gourmand" : "🔎 Discovery Walk & Gourmet Workshop"}</h3>
            <div className="space-y-4 text-lg text-stone font-montserrat leading-relaxed">
              ...
              <p className="font-semibold text-stone-dark">{locale === 'fr' ? "La sortie se terminera par une pause gourmande : dégustation de noix fraîches et d'une tarte aux noix locale, partagée tous ensemble autour d'un moment chaleureux." : "The outing will end with a gourmet break: tasting fresh walnuts and a local walnut tart, shared together in a warm moment."}</p>
            ...
            <div className="bg-white rounded-lg p-8 shadow-xl overflow-x-auto">
              <table className="w-full font-montserrat">
                <tbody className="divide-y divide-stone-light">
                  <tr><td className="py-4 px-4 font-semibold text-stone-dark">📅 {locale === 'fr' ? "Date" : "Date"}</td><td className="py-4 px-4 text-stone">{locale === 'fr' ? "Dimanche 18 octobre 2026" : "Sunday, October 18, 2026"}</td></tr>
                  ...
                  <tr><td className="py-4 px-4 font-semibold text-stone-dark">📍 {locale === 'fr' ? "Lieu" : "Location"}</td><td className="py-4 px-4 text-stone">{locale === 'fr' ? "Vérel-Pragondran (près d'Aix-les-Bains)" : "Vérel-Pragondran (near Aix-les-Bains)"}</td></tr>
                  <tr><td className="py-4 px-4 font-semibold text-stone-dark">📏 {locale === 'fr' ? "Distance" : "Distance"}</td><td className="py-4 px-4 text-stone">{locale === 'fr' ? "Environ 4 km" : "About 4 km"}</td></tr>
                  <tr><td className="py-4 px-4 font-semibold text-stone-dark">⏱ {locale === 'fr' ? "Durée totale" : "Total duration"}</td><td className="py-4 px-4 text-stone">{locale === 'fr' ? "Environ 2h30 à 3h (balade + atelier + dégustation)" : "About 2h30 to 3h (walk + workshop + tasting)"}</td></tr>
                  <tr><td className="py-4 px-4 font-semibold text-stone-dark">⛰ {locale === 'fr' ? "Dénivelé positif" : "Elevation gain"}</td><td className="py-4 px-4 text-stone">{locale === 'fr' ? "+70 m environ" : "+70 m approximately"}</td></tr>
                  <tr><td className="py-4 px-4 font-semibold text-stone-dark">⚙ {locale === 'fr' ? "Difficulté" : "Difficulty"}</td><td className="py-4 px-4 text-stone">{locale === 'fr' ? "Facile – accessible à tous" : "Easy – accessible to all"}</td></tr>
                  ...
                  <tr><td className="py-4 px-4 font-semibold text-stone-dark">🎒 {locale === 'fr' ? "À prévoir" : "What to bring"}</td><td className="py-4 px-4 text-stone">{locale === 'fr' ? "Chaussures de marche, eau, coupe-vent, curiosité" : "Hiking shoes, water, windbreaker, curiosity"}</td></tr>
                </tbody>
              </table>
            </div>
          </motion.div>
